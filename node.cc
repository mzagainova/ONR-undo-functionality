...
Node::Node(NodeId_t name, NodeList peers, NodeList children, NodeId_t parent,
  ...
  ros::ServiceClient dropClient = local_.serviceClient<table_task_sim::DropObject>("/drop_service");
  dropClient_pntr = &dropClient;
...
void Node::undoCallback(ConstControlMessagePtr_t msg) {
  ...
  // call drop service in simulator
  table_task_sim::DropObject srv;
  srv.request.robot_id = state_.owner.robot;

  if(ros::service::call("/drop_service", srv)) {
    ROS_INFO("SUCCESS %d", srv.response.result);
  }
...
void Node::PublishStatus() {
  ...
  msg.parent_type = state_.parent_type;
  msg.tempDeactivate = state_.tempDeactivate;
